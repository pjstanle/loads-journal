...
max_damage_array = [0.065,0.06,0.055,0.05]
for k = 1:length(max_damage_array)
    ...
    # start_x = readdlm("x_40_small_26.txt")[:,1]
    # start_y = readdlm("y_40_small_26.txt")[:,1]
    ...
    open("x_40_small2_loads_$max_damage.txt", "w") do io
            writedlm(io, opt_x)
    end

    open("y_40_small2_loads_$max_damage.txt", "w") do io
            writedlm(io, opt_y)
    end
end
