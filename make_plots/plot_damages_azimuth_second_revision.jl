...
turb_samples_big = spl.(x_spl_big)
...
for k = 1:length(sep_arr)
        ...
        for i = 1:L
            ...
            damage_model2[i] = ff.get_single_damage_FINAL(turbine_x,turbine_y,turbine_z,rotor_diameter,hub_height,turbine_yaw,turbine_ai,sorted_turbine_index,turbine_ID,state_ID,nCycles,az_arr,
        ...
        for i = 1:L
            ...
            damage_model100[i] = ff.get_single_damage_FINAL(turbine_x,turbine_y,turbine_z,rotor_diameter,hub_height,turbine_yaw,turbine_ai,sorted_turbine_index,turbine_ID,state_ID,nCycles,az_arr,
                                turb_samples_big,pitch_func,ff.GaussianTI,r,sections,Rhub,Rtip,precone,tilt,windresource,model_set,
                                turbine_velocities, turbine_ct,turbine_local_ti;
                                Nlocs=Nlocs,fos=2.0,div_sigma=ds,div_ti=dt)
        ...
        if sep == 4.0 && ws == 11.0
                println("damage_model2: ", repr(damage_model2))
                println("damage_model100: ", repr(damage_model100))
                subplot(131)
                plot(sweep,damage_model2)
        ...
        elseif sep == 10.0 && ws == 11.0
                ...
                println("damage_model100: ", repr(damage_model100))
                subplot(133)
                plot(sweep,damage_model2,label="2 azimuth angles")
                plot(sweep,damage_model100,"--",label="100 azimuth angles")
                xlabel("offset (D)")
...
# savefig("damage_azimuth_second_revision.pdf",transparent=true)
