...
function SOWFA_damage(turbine_y,nCycles,az_arr,turb_samples,pitch_func,turbulence_func,U_func,
    r,sections,Rhub,Rtip,precone,tilt;Nlocs=20,fos=1.15)
        ...
        # end
        Omega = 7.55*turbine_velocity/(126.4/2.0)
...
L = 100
sweep = range(-1.2,stop=1.2,length=L)
...
for k = 1:length(sep_arr)
    for j = 1:length(ws_arr)
        ...
        for i = 1:L
            turbine_y = sweep[i]*rotor_diameter[1]

            damage_model[i] = SOWFA_damage(turbine_y,nCycles,az_arr,turb_samples,pitch_func,turbulence_func,U_func,
                                                r,sections,Rhub,Rtip,precone,tilt;Nlocs=50,fos=1.15)
...
suptitle("high turbulence intensity: 8%\nSOWFA profiles",fontsize=10,y=0.99)
subplots_adjust(top=0.91,bottom=0.1,left=0.13,right=0.99,wspace=0.1,hspace=0.1)
# savefig("damage_highTI_SOWFA.pdf",transparent=true)
